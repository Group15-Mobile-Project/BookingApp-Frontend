...
import { getNotificationsByHostAction, getNotificationsByTenantAction } from '../../Store/Actions/NotificationAction';
...
const HostNotificationScreen = () => {
    ...
    const loadNotifications = useCallback(async () => {
        if(authUser) {
            ...
            await dispatch(getNotificationsByHostAction() as any)
            setIsRefreshing(false)
        }
    }, [authUser, dispatch])

    const handleRenderItem: ListRenderItem<any> = ({item}: {item: NOTIFICATION}) => (
        <HostNotificationCard notify={item}></HostNotificationCard>
    )
...
export default HostNotificationScreen
