...
const SignUpScreen = () => {
    ...
    const submitFunction = async () => {
        if(username && username.length > 0 && password && password.length > 0 && confirmPassword && confirmPassword.length > 0 && email && email.length > 0 ) {
           await  dispatch(Register({username, email,  password, confirmPassword}) as any)
           setUsername("")
           setPassword("")
           setImageurl(null)
           setConfirmPassword("")
  ...
  return (
   <KeyboardAvoidingView style={tw('flex-1')}>
    ...
    <SafeAreaView style={tw('flex-1 items-center justify-center px-4')}>    
      ...
      <Button  color="#FF5A5F" containerStyle={tw('w-full rounded-lg mb-6')} size='lg' title='Sign Up' onPress={submitFunction}></Button>
