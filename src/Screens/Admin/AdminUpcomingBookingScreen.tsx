...
import { getUpcomingBookingByHomeAction, getUpcomingBookingByHostAction, getUpcomingBookingByTenantAction } from '../../Store/Actions/BookingAction';
...
const AdminUpcomingBookingScreen = ({userId, homeId}: {userId?: number, homeId?: number}) => {
  ...
  const loadUpcomingBookingsByTenants = useCallback(async () => {
      ...
      if(userId) {
        await dispatch(getUpcomingBookingByTenantAction(userId) as any);
      } else if(homeId) {
        await dispatch(getUpcomingBookingByHomeAction(homeId) as any);
      }
    ...
    const handleRenderItem: ListRenderItem<any> = ({item}: {item: BOOKING}) => (
      <AdmingBookingCard booking={item} upcoming={true} onPress={() => navigation.navigate("DetailBookingScreenAdmin", {bookingId: item?.id})}></AdmingBookingCard>
  )
...
export default AdminUpcomingBookingScreen
