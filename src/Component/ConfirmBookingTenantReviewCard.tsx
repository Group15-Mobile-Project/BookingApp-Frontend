...
import { HOME, HOMEREVIEW, TENANTREVIEW } from '../Model'
...
const ConfirmBookingTenantReviewCard = ({item}: {item: TENANTREVIEW}) => {
    ...
    return (
      <View style={[tw('border border-gray-400 rounded-lg py-2 px-2 mx-2'), {width: windownWith - 100}]}>
        <View style={tw('flex-row items-center justify-start')}>
          <Image source={{uri: HOST_URL + "/api/images/image/" + item?.host?.user?.imgUrls}} style={[tw('rounded-full mr-4'), {width: 40, height: 40, resizeMode: 'cover'}]}></Image>      
          <View style={tw(' items-start justify-start')}>
            <Text style={tw(' text-lg text-black font-bold')}>{item?.host?.user?.username}</Text>
...
export default ConfirmBookingTenantReviewCard
